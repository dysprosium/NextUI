...
void CFG_defaults(NextUISettings *cfg)
{
    ...
    NextUISettings defaults = {
        ...
        .autoPowerOffTimeoutSecs = CFG_DEFAULT_AUTOPOWEROFFTIMEOUTSECS,
...
void CFG_init(FontLoad_callback_t cb, ColorSet_callback_t ccb)
{
    ...
    else
    {
        ...
        while (fgets(line, sizeof(line), file))
        {
            ...
            if (sscanf(line, "autoPowerOffTimeout=%i", &temp_value) == 1)
            {
                if (temp_value >= 0)
                    CFG_setAutoPowerOffTimeoutSecs((uint32_t)temp_value);
                continue;
            }
...
uint32_t CFG_getAutoPowerOffTimeoutSecs(void)
{
    return settings.autoPowerOffTimeoutSecs;
}

void CFG_setAutoPowerOffTimeoutSecs(uint32_t secs)
{
    settings.autoPowerOffTimeoutSecs = secs;
    CFG_sync();
}
...
void CFG_get(const char *key, char *value)
{
    ...
    else if (strcmp(key, "autoPowerOffTimeout") == 0)
    {
        sprintf(value, "%u", CFG_getAutoPowerOffTimeoutSecs());
    }
...
void CFG_sync(void)
{
    ...
    fprintf(file, "autoPowerOffTimeout=%u\n", settings.autoPowerOffTimeoutSecs);
