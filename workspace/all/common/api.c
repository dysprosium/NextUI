		...
		if (sleepDelay > 0)
		{
			...
			{ // increased to two minutes
				...
				if (PLAT_supportsDeepSleep())
				{
					...
					else if (ret == PWR_DEEPSLEEP_AUTO_POWEROFF)
					{
						return PWR_DEEPSLEEP_AUTO_POWEROFF;
					}
					else
					{
...
void PWR_sleep(void)
{
	...
	if (wake_result == PWR_DEEPSLEEP_AUTO_POWEROFF)
	{
		LOG_info("automatic deep-sleep timeout reached - powering off\n");
		PWR_powerOff(0);
		return;
	}
	PWR_exitSleep();
...
int PWR_deepSleep(void)
{
	...
	if (exists(suspend_path))
	{
		...
		uint32_t auto_poweroff_secs = pwr.can_poweroff ? CFG_getAutoPowerOffTimeoutSecs() : 0;
		unsetenv("NEXTUI_AUTO_POWEROFF_SECS");
		if (auto_poweroff_secs > 0)
		{
			char timeout[16];
			snprintf(timeout, sizeof(timeout), "%u", auto_poweroff_secs);
			if (setenv("NEXTUI_AUTO_POWEROFF_SECS", timeout, 1) != 0)
			{
				LOG_error("failed to configure automatic poweroff timeout: %d\n", errno);
			}
		}
